...
class TestValidater(TestCase):
    ...
    def test_check_validate_guid_fail(self):
        with self.assertRaisesRegex(argparse.ArgumentTypeError,f"GUID should be 36 characters displayed in five groups separated by a dash in the format [XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX]. Hexdigits are not allowed"):

...
from ctypes import *
from inbm_vision_lib.xlink.xlink_windows_wrapper import XlinkWindowsWrapper
...
class TestXlinkWindowsWrapper(TestCase):

    @patch('ctypes.CDLL.__init__', return_value=None)
    @patch('threading.Thread.start')
    def setUp(self, t_start, lib_init) -> None:
        self.receive = MagicMock()
        self.xlink_wrapper = XlinkWindowsWrapper(self.receive, UNSECURED_XLINK_CHANNEL, 0, True)
    ...
    @patch('inbm_vision_lib.xlink.xlink_windows_wrapper.XlinkWindowsWrapper.boot_device', return_value=X_LINK_SUCCESS)
    ...
    @patch.object(MockXlinkLib, 'xlink_write_data', return_value=X_LINK_SUCCESS)
    @patch('time.sleep', return_value=None)
    def test_send_file_multiple_chunk(self, time, write, check_status, get_size, open_file, mock_exists, mock_islink):
        self.xlink_wrapper.xlink_init_status_success = True
        self.xlink_wrapper.send_file("123")
    ...
    @patch('inbm_vision_lib.xlink.xlink_windows_wrapper.XlinkWindowsWrapper._xlink_release_data')
    ...
    @patch('os.path.getsize', return_value=100)
    @patch('inbm_vision_lib.xlink.xlink_windows_wrapper.XlinkWindowsWrapper._download_normal_file')
    @patch('inbm_vision_lib.xlink.xlink_windows_wrapper.XlinkWindowsWrapper._xlink_release_data')
    @patch('inbm_vision_lib.xlink.ixlink_wrapper.IXlinkWrapper._check_directory')
    def test_receive_file(self, check_dir, release_data, download_file, get_size):
        def stop_listen():
            sleep(3)
            self.xlink_wrapper._running = False
        thread = threading.Thread(target=stop_listen)
        ...
        filename = self.xlink_wrapper.receive_file("mock_path")
        assert release_data.call_count == 3
        check_dir.assert_called()
        download_file.assert_called()
        get_size.assert_called()
        self.assertEqual(filename, "1")

    @patch.object(MockXlinkLib, 'xlink_read_data', return_value=X_LINK_SUCCESS)
    @patch('inbm_vision_lib.xlink.xlink_windows_wrapper.XlinkWindowsWrapper._xlink_release_data')
    def test_download_large_file(self, release_data, xlink_read_data):
        with patch("builtins.open", mock_open(read_data="20201209083327")) as mock_file:
            self.xlink_wrapper._download_large_file("mock_path", 2)

        assert xlink_read_data.call_count == 2
        release_data.assert_called()

    @patch.object(MockXlinkLib, 'xlink_read_data', return_value=X_LINK_SUCCESS)
    def test_download_normal_file(self, xlink_read_data):
        with patch("builtins.open", mock_open(read_data="20201209083327")) as mock_file:
            self.xlink_wrapper._download_normal_file("mock_path")

        assert xlink_read_data.call_count == 1

...
class TestVersion(TestCase):
    ...
    @patch('inbm_lib.version._read_inbm_vision_version_file_node')
    @patch('inbm_lib.version._read_inbm_version_file')
    def test_get_friendly_inbm_version_commit(self, tc, bc_node, bc_host):
    ...
    @patch('inbm_lib.version._read_inbm_vision_version_file_node')
    @patch('inbm_lib.version._read_inbm_version_file')
    def test_get_friendly_inbm_vision_version_commit(self, tc, bc_node, bc_host):
        tc.return_value = INBM_VERSION_TEXT
        bc_node.return_value = INBM_VERSION_TEXT_NODE
        bc_host.return_value = INBM_VERSION_TEXT_HOST

        # Use host version when both node+host present
        self.assertEquals("Intel(R) Manageability Vision host version 2.3.4 (aaabbbc)", get_friendly_inbm_vision_version_commit())


    @patch('inbm_lib.version._read_inbm_vision_version_file_host')
    @patch('inbm_lib.version._read_inbm_vision_version_file_node')
    @patch('inbm_lib.version._read_inbm_version_file')
    def test_get_friendly_inbm_vision_version_commit_node_only(self, tc, bc_node, bc_host):
        tc.return_value = INBM_VERSION_TEXT
        bc_node.return_value = INBM_VERSION_TEXT_NODE
        bc_host.return_value = None

        # Use node version.
        self.assertEquals("Intel(R) Manageability Vision node version 3.4.5 (fffeeed)", get_friendly_inbm_vision_version_commit())

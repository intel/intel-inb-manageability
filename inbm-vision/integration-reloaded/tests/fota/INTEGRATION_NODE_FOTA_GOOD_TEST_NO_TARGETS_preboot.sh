...
(if listen_reboot_message | grep Rebooting ; then
  echo Integration Node Fota good test with no targets preboot good pass.
else
  echo Test failed to detect rebooting message...
  cat /tmp/listen_event_last_log
  echo Error in Integration Node fota good test with no targets preboot.  Showing recent journalctl.
...
test_echo Triggering INTEGRATION_NODE_FOTA_GOOD_TEST_NO_TARGETS_preboot test
inbc fota -p ${VAGRANT_INPUT_PATH}/BIOSUPDATE.tar -pr "Broxton P" -m testmanufacturer -v "Intel Corp." -pr "Broxton P" -s `python3 /scripts/create_signature.py ${VAGRANT_INPUT_PATH}/succeed_rpm_key.pem ${VAGRANT_INPUT_PATH}/BIOSUPDATE.tar  1234`
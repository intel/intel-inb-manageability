...
## 📝 Note
1. INBC supports FOTA, SOTA, POTA, ConfigUpdate(Set,Get) on an Edge device. Use the **'--nohddl'** flag to target an Edge device.  This requires downloading from a remote source.
...
inbc get [--nohddl]
   {--path, -p KEY_PATH;...} 
   [--targettype, -tt NODE | VISION | NODE_CLIENT; default=None]
   [--target, -t TARGETS...; default=None]
```   
### Examples
#### Edge Device on Yocto OS
```
inbc get --nohddl --path -p publishIntervalSeconds
```
#### Edge Device on Ubuntu
```
inbc get --nohddl --path -p publishIntervalSeconds
...
#### Edge Device on Yocto OS
```
inbc set --nohddl --path -p maxCacheSize:100
```
#### Edge Device on Ubuntu
```
inbc set --nohddl --path -p maxCacheSize:100

...
func (f FakeCommandExec) Run(command string, directory string, args []string) error {
	return f.Err
}

// CombinedOutput makes an actual call to exec.Command to run the command and get the output.
func (f FakeCommandExec) CombinedOutput(command string, directory string, args []string) ([]byte, error) {

...
class TestAOTA(TestCase):
    ...
    def test_failure_one_application_update(self, mock_create_repo, mock_verify_source, mock_get_file, mock_osdir) -> None:
    ...
    def test_application_update_fail(self) -> None:
    ...
    def test_raise_when_application_update_fails(self, check_os, mock_cleanup, mock_shell, mock_get_repo, mock_platform,
                                                 mock_create_repo,
                                                 mock_verify_source, mock_get_file, mock_osdir) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    def test_fails_install_with_resource_type_empty(self, mock_verify_source) -> None:
    ...
    def test_fails_source_verification_check(self, mock_verify_source) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    def test_fails_package_name_load(self, mock_verify_source) -> None:
    ...
    def test_success_load(self, mock_create_repo, mock_verify_source, mock_get_file, mock_image_load, mock_osdir) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    def test_fail_load(self, mock_verify_source, mock_get_file, mock_image_load, mock_osdir) -> None:
        aota = TestAOTA._build_aota(uri='file://sample/test.tar',
    ...
    def test_compose_pull_success(self, mock_download, mock_pull) -> None:
    ...
    def test_http_server_username_password(self, mock_get_repo, mock_verify_source) -> None:
    ...
    def test_docker_pull_no_username(self) -> None:
        aota = TestAOTA._build_aota(app_type='docker', cmd='pull', container_tag="hello",
    ...
    def test_docker_pull_called(self, mock_docker_pull, mock_trtl_pull) -> None:
    ...
    @patch('inbm_lib.trtl.Trtl.image_pull_public', return_value=["", "", 0])
    @patch('inbm_lib.trtl.Trtl.remove_old_images', return_value=None)
    def test_docker_pull_public_success(self, mock_trtl_old, mock_trtl_pull) -> None:
        aota = TestAOTA._build_aota(app_type='docker', cmd='pull', container_tag="hello",
    ...
    def test_application_up_fail(self) -> None:
    ...
    def test_compose_up_fail_one(self) -> None:
    ...
    def test_compose_update_fail(self) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    def test_compose_docker_update_fail(self, mock_verify_source) -> None:
    ...
    def test_docker_compose_down_function_called(self, mock_docker_down) -> None:
        aota = TestAOTA._build_aota(app_type='compose', cmd='down', container_tag="hello",
                                    docker_registry="https://www.example.com/")
        aota.run()
        mock_docker_down.assert_called_once()

    def test_docker_down_fail(self) -> None:
    ...
    def test_docker_pull_public_fail(self, mock_trtl) -> None:
        aota = TestAOTA._build_aota(app_type='docker', cmd='pull', container_tag="hello",
                                    docker_registry='https://docker.hub')
    ...
    @patch('dispatcher.aota.checker.verify_source')
    @patch('dispatcher.aota.aota_command.get', return_value=dummy_success)
    def test_docker_login_no_password(self, mock_get_repo, mock_verify_source) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    @patch('dispatcher.aota.aota_command.get', return_value=dummy_success)
    def test_docker_login_no_username(self, mock_get_repo, mock_verify_source) -> None:
        aota = TestAOTA._build_aota(container_tag='abc', uri='file://sample/test.tar.gz', cmd='pull',
                                    app_type='compose',
                                    need_repo=False,
                                    docker_registry='amr-registry-pre.caas.intel.com',
    ...
    @patch('dispatcher.aota.checker.verify_source')
    @patch('dispatcher.aota.aota_command.get', return_value=dummy_success)
    def test_docker_login_no_registry_url(self, mock_get_repo, mock_verify_source) -> None:
        aota = TestAOTA._build_aota(container_tag='abc', uri='file://sample/test.tar.gz', cmd='pull',
                                    app_type='compose',
    ...
    @patch('dispatcher.aota.aota_command.get', return_value=Result(200, "OK"))
    def test_compose_up_success(self, mock_start, mock_get, mock_verify) -> None:
        try:
    ...
    def test_raises_compose_up_unable_to_get_package(self, mock_start, mock_get, mock_verify) -> None:
    ...
    def test_raise_When_docker_down_missing_container_tag(self, mock_down) -> None:
    ...
    def test_compose_remove_success(self, mock_down, mock_remove) -> None:
    ...
    @patch.object(Trtl, 'stop_all', return_value=("", "", 0))
    def test_docker_remove(self, mock_stop, mock_remove) -> None:
        aota = self._build_aota(container_tag='abc', cmd='remove',
    ...
    @patch.object(Trtl, 'stop_all', return_value=("", "", 0))
    def test_docker_down(self, mock_trtl) -> None:
    ...
    @patch.object(Trtl, 'stop_all', return_value=("", "", 0))
    def test_docker_remove_missing_container_tag(self, mock_stop) -> None:
    ...
    def test_docker_stats_success(self, mock_stats) -> None:
        aota = self._build_aota(cmd='stats', app_type='docker')
        res = aota.run()
        self.assertEqual(None, res)

    @patch('dispatcher.aota.aota_command.Docker.stats')
    def test_docker_stats_failed(self, mock_stats) -> None:
        aota = self._build_aota(cmd='stats', app_type='docker')
        aota.run()
        mock_stats.assert_called_once()

    @patch('os.rmdir')
    def test_compose_pull_no_uri(self, mock_rmdir) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    def test_raise_when_container_tag_empty(self, mock_verify_source) -> None:
    ...
    def test_fails_source_verification_check_http(self, mock_verify_source) -> None:
    ...
    @patch('dispatcher.aota.checker.verify_source')
    def test_run_command_compose_up_no_error(self, mock_verify_source, mock_composeup, mock_rmdir) -> None:
    ...
    def test_run_command_compose_up_error(self, mock_image_pull_private, mock_verify_source) -> None:
    ...
    def test_compose_list(self, mock_list) -> None:
    ...
    def test_compose_list_fail(self, mock_list) -> None:
    ...
    def test_run_cmd_down(self) -> None:
    ...
    def test_run_cmd_import_with_tag(self, mock_image_import, mock_rmdir, mock_verify, mock_remove) -> None:
    ...
    def test_run_cmd_import_clean_up_called(self, mock_trtl_cntr, mock_delete, mock_rmdir, mock_verify) -> None:
        aota = self._build_aota(cmd='import', app_type='docker',
    ...
    @patch('os.rmdir')
    @patch('dispatcher.aota.aota_command.TrtlContainer.image_import')
    def test_run_cmd_import_without_tag(self, mock_image_import, mock_rmdir) -> None:
    ...
    def test_raise_compose_pull_missing_container_tag(self, mock_image_pull_public, mock_compose_download) -> None:
    ...
    @patch('dispatcher.aota.aota_command.DockerCompose.list')
    def test_run_command_list_success(self, mock_cmd_list) -> None:
        aota = self._build_aota(cmd='list', app_type='compose', container_tag="abc")
        aota.run()
        mock_cmd_list.assert_called_once()

    def test_run_command_remove_raise_error(self) -> None:
    ...
    def test_run_command_remove_function_called(self, mock_a, mock_image_remove_all, mock_docker_down) -> None:
        aota = self._build_aota(cmd='remove', app_type='docker')
        aota.run()
    ...
    def test_application_centos_driver_update_raise_error_not_in_container(self, check_url, mock_detect_os) -> None:
    ...
    def test_application_centos_driver_update_raise_error_if_inb_driver_folder_not_found(self, detect_os,
                                                                                         is_inside_container, create_repo, verify_source, get, mock_cleanup) -> None:
        aota = self._build_aota(cmd='update', app_type='application',
                                uri="http://example.com", device_reboot="Yes")
        self.assertRaises(AotaError, aota.run)
    ...
    def test_application_centos_driver_update_raise_pass(self, detect_os, mock_detect_os, create_repo, listdir, mock_move,
                                                         support_driver, run, mock_is_rpm_file_type, mock_get, mock_verify, mock_resource) -> None:

...
class TestOsFactory(TestCase):
    ...
    def test_get_factory_linux(self):
        assert type(OsFactory.get_factory("Linux", fake_ota_resource,
                                          self.mock_disp_obj, self.mock_disp_broker)) is LinuxFactory

    def test_get_factory_windows(self):
        assert type(OsFactory.get_factory("Windows", fake_ota_resource,
                                          self.mock_disp_obj, self.mock_disp_broker)) is WindowsFactory

    def test_raise_error_unsupported_os(self):
        self.assertRaises(ValueError, OsFactory.get_factory, "MacOS",
                          fake_ota_resource, self.mock_disp_obj, self.mock_disp_broker)

    def test_create_linux_upgrade_checker(self):
        assert type(OsFactory.get_factory("Linux", fake_ota_resource, self.mock_disp_obj, self.mock_disp_broker).create_upgrade_checker()) \
            is LinuxUpgradeChecker

    def test_create_windows_upgrade_checker(self):
        assert type(OsFactory.get_factory("Windows", fake_ota_resource, self.mock_disp_obj, self.mock_disp_broker).create_upgrade_checker()) \
            is WindowsUpgradeChecker

    def test_create_linux_installer(self):
        assert type(OsFactory.get_factory("Linux", fake_ota_resource,
                                          self.mock_disp_obj, self.mock_disp_broker).create_installer(MemoryRepo("test"),
    ...
    def test_create_linux_rebooter(self):
        assert type(OsFactory.get_factory("Linux", fake_ota_resource, self.mock_disp_obj, self.mock_disp_broker).create_rebooter()) \
            is LinuxRebooter

    def test_create_windows_rebooter(self):
        assert type(OsFactory.get_factory("Windows", fake_ota_resource, self.mock_disp_obj, self.mock_disp_broker).create_rebooter()) \
            is WindowsRebooter

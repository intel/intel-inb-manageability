...
class TestRemediationManager(TestCase):
    ...
    @patch('inbm_lib.trtl.Trtl.StopAll')
    @patch('inbm_lib.trtl.Trtl.remove_container')
    def test_return_container_no_errors(self, mock_remove_container, mock_StopAll, mock_call_telemetry, mock_image, mock_remove, mock_list):
        try:
            mock_StopAll.return_value = (None, None, 0)

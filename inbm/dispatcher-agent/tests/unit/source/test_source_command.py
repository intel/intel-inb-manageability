...
def test_do_source_command_list(mocker, xml_handler_factory, xml, patch_target, return_value, expected_message):
    ...
    mocker.patch(
        patch_target,
        return_value=mock_source_manager
    )
...
def test_do_source_os_command_remove(mocker, xml_handler_factory):
    ...
    mock_os_source_manager = mocker.Mock()
    mock_os_source_manager.remove.return_value = None

    mocker.patch(
        'dispatcher.source.source_command.create_os_source_manager',
        return_value=mock_os_source_manager
    )
    ...
    mock_os_source_manager.remove.assert_called_once_with(SourceParameters(sources=['source1', 'source2']))
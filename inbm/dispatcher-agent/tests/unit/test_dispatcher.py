...
class TestDispatcher(TestCase):
    ...
    def test_aota_fails_on_no_trtl_framework(self,
                                             mock_trtl_path: Any,
                                             mock_ota_thread: Any,
                                             mock_pre_install_check: Any) -> None:
    ...
    def test_do_install_ota_error_result_succeeds(self, mock_workload_orchestration_func, MockXmlHandler,
        ...
        d = TestDispatcher._build_dispatcher()
        with patch('dispatcher.validators.validate_xml_manifest', return_value=parsed_head):
            ...
            xml = '<?xml version="1.0" encoding="UTF-8"?>' \
                  ...
                  '<version>1.0</version><containerTag>defg</containerTag>' \
                  '</aota></type></ota></manifest>'
            d.do_install(xml, schema_location=TEST_SCHEMA_LOCATION)
    ...
    def test_do_install_pota_do_ota_func_called(self, MockXmlHandler) -> None:
        ...
        resource = {'fota': ' ', 'sota': ' '}
        d = TestDispatcher._build_dispatcher()
        with patch('dispatcher.dispatcher_class.validate_xml_manifest', return_value=parsed_head):
            d._send_result = Mock()  # type: ignore[method-assign]
    ...
    def test_dispatcher_state_file_info_sota(self, mock_disp_state_file_exist, mock_consume_disp_file, mock_invoke_sota) -> None:
    ...
    def test_dispatcher_state_file_info_no_restart_reason(self, mock_disp_state_file_exist, mock_consume_disp_file) -> None:
        d = TestDispatcher._build_dispatcher()
        try:
            d.check_dispatcher_state_info()
    ...
    def test_dispatcher_state_file_info_sota_without_restart_reason(self, mock_disp_state_file_exist,
                                                                    mock_consume_disp_file, mock_invoke_sota) -> None:
        mock_install = MockInstallCheckService()
        d = TestDispatcher._build_dispatcher(install_check=mock_install)
        d.check_dispatcher_state_info()
        self.assertTrue(mock_install.install_check_called())
    ...
    def test_dispatcher_device_tree_called_on_disp_state(self, mock_consume_disp_file, mock_disp_state_file_exist,
                                                         mock_devicetree, mock_dmi_path, mock_send_result) -> None:
        d = TestDispatcher._build_dispatcher()
        d.check_dispatcher_state_info()
    ...
    def test_do_install_can_call_do_source_command(self,
                                                   ...
                                                   m_sub: Any,
                                                   m_connect: Any,
                                                   mock_do_source_command: Any) -> None:
        xml = """\

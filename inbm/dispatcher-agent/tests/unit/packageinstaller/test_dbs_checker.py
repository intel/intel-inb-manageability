...
class TestDbsChecker(TestCase):
    ...
    def test_return_build_result_success(self, mock_trtl_container):
        ...
        fails = "Failures in: "
        try:
            DbsChecker(MockDispatcherCallbacks.build_mock_dispatcher_callbacks(), MockDispatcherBroker.build_mock_dispatcher_broker(), mock_trtl_container,
    ...
    def test_find_current_container(self, mock_trtl_container):
        mock_trtl = MockTrtl(smart_error=True)
        container = DbsChecker(MockDispatcherCallbacks.build_mock_dispatcher_callbacks(), MockDispatcherBroker.build_mock_dispatcher_broker(), mock_trtl_container,
                               mock_trtl, "sample-container", 0, ConfigDbs.ON). \

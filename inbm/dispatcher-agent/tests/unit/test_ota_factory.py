...
class TestOtaFactory(TestCase):
    ...
    def test_raise_error_unsupported_ota(self):
        self.assertRaises(ValueError, OtaFactory.get_factory,
                          "IOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), True)

    def test_create_fota_parser(self):
        assert type(OtaFactory.get_factory(
            "FOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), ConfigDbs.ON).create_parser()) is FotaParser

    def test_create_sota_parser(self):
        assert type(OtaFactory.get_factory(
            "SOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), ConfigDbs.ON).create_parser()) is SotaParser

    def test_create_aota_parser(self):
        assert type(OtaFactory.get_factory(
            "AOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), ConfigDbs.ON).create_parser()) is AotaParser

    def test_create_fota_thread(self):
        assert type(OtaFactory.get_factory(
            "FOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), ConfigDbs.ON).create_thread('abc')) is FotaThread

    def test_create_sota_thread(self):
        assert type(OtaFactory.get_factory(
            "SOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), ConfigDbs.ON).create_thread('abc')) is SotaThread

    def test_create_aota_thread(self):
        assert type(OtaFactory.get_factory(
            "AOTA", "remote", self.mock_disp_obj, None, MockInstallCheckService(), ConfigDbs.ON).create_thread('abc')) is AotaThread

...
class TestGranularLogHandler(testtools.TestCase):
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_insufficient_storage_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_ut_write_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_ut_apply_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_ut_commit_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_rsauthentication_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
        update_logger.error = 'Failed to access URI:Status code for https://files-rs.internal.ledgepark.intel.com/repository/TiberOS/TiberOS-nonRT/tiber-readonly-1.0.20241120.0715.raw.gz is 0. Invalid URI or Token might be expired.'
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_signature_check_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
    ...
    @patch('json.load', return_value={"UpdateLog":[]})
    @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
    def test_save_granular_in_tiberos_with_critical_service_fail_log(self, mock_run, mock_load, mock_dump, mock_exists, mock_truncate) -> None:
        update_logger = UpdateLogger("SOTA", "metadata")
        update_logger.detail_status = FAIL
        ...
        @patch('json.load', return_value={"UpdateLog": []})
        @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
        def test_save_granular_in_tiberos_with_trusted_repository_fail_log(self, mock_run, mock_load, mock_dump,
        ...
        @patch('json.load', return_value={"UpdateLog": []})
        @patch('inbm_common_lib.shell_runner.PseudoShellRunner.run', return_value=("tiber", "", 0))
        def test_save_granular_in_tiberos_with_unspecified_fail_log(self, mock_run, mock_load, mock_dump,
                                                                         mock_exists, mock_truncate) -> None:
            update_logger = UpdateLogger("SOTA", "metadata")
            update_logger.detail_status = FAIL

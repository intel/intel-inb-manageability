...
class TestLogHelper(unittest.TestCase):
    def test_succeeds_return_file_when_upgrade_a(self) -> None:
        assert (get_log_destination("Y", "upgrade") == 'FILE')

    def test_succeeds_return_file_when_log_to_file(self) -> None:
        assert (get_log_destination("Y", "update") == 'FILE')

    def test_succeeds_return_file_when_upgrade_b(self) -> None:
        assert (get_log_destination("N", "upgrade") == 'FILE')

    def test_succeeds_return_cloud_when_not_upgrade_and_not_log_to_file(self) -> None:
        assert (get_log_destination("N", "update") == 'CLOUD')

    def test_succeeds_return_true_valid_yes(self) -> None:
        self.assertTrue(_is_valid_yes_value("Yes"))

    def test_succeeds_return_false_valid_yes(self) -> None:
        self.assertFalse(_is_valid_yes_value("No"))

    @mock.patch('dispatcher.sota.log_helper.open')
    def test_succeeds_log_command_error_with_file_destination_read(self, mock_open) -> None:
    ...
    @mock.patch('dispatcher.sota.log_helper.open')
    def test_succeeds_log_command_error_without_file_destination_read(self, mock_open) -> None:
        mock_open.return_value = File('FILE CONTENTS')
        log_command_error(cmd=mock.Mock(), cmd_index=1, err='err', output='output',

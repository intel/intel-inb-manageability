...
def tiber_download(dispatcher_broker: DispatcherBroker, uri: CanonicalUri,
    ...
    try:
        with requests.get(url=uri.value, headers=headers, verify=_verify_ssl(uri), stream=True) as response:
...
def is_enough_space_to_download(manifest_uri: str,
    ...
    try:
        ...
        with requests.get(url=manifest_uri, headers=headers, verify=_verify_ssl(CanonicalUri(manifest_uri)), stream=True) as response:
            response.raise_for_status()

...
after_install() {
    ...
    chmod 644 $SYSTEMD_DIR/inbm-cloudadapter.service

    if [ "$(cat /proc/1/comm)" == "systemd" ]; then
    	echo "Found systemd"
		echo "Activating cloudadapter's apparmor policies"
		apparmor_reload /etc/apparmor.d/usr.bin.inbm-cloudadapter
    	# Reload daemon to pick up new changes
		systemctl daemon-reload
		echo "Ran systemctl daemon-reload"
	else
    	echo "No systemd found; run cloudadapter manually"
    fi
}

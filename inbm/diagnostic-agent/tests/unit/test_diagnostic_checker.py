...
class TestDiagnosticChecker(TestCase):
    ...
    def test_check_sw_mandatory_notsplit(self, m_sub, m_connect) -> None:
        self.dc.sw_list = ' trtl docker '
        self.assertEqual(self.dc._check_sw_mandatory_list('docker'), False)

    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_check_sw_mandatory_list_success(self, m_sub, m_connect) -> None:
        self.dc.sw_list = '\n  trtl\n     docker\n'
        self.assertEqual(self.dc._check_sw_mandatory_list('docker'), True)

    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_check_sw_mandatory_missing_sw_in_list(self, m_sub, m_connect) -> None:
        self.dc.sw_list = '  trtl\n  '
        self.assertEqual(self.dc._check_sw_mandatory_list('docker'), False)

    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_check_sw_mandatory_wrong_sw(self, m_sub, m_connect) -> None:
        self.dc.sw_list = '  trtl\n docker\n '
        self.assertEqual(self.dc._check_sw_mandatory_list('test'), False)

    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_check_sw_mandatory_None_sw_list(self, m_sub, m_connect) -> None:
        self.dc.sw_list = None
        self.assertEqual(self.dc._check_sw_mandatory_list('test'), False)

    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_check_sw_mandatory_list_NoneSW(self, m_sub, m_connect) -> None:
        self.dc.sw_list = '  trtl\n docker\n '
    ...
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_set_configuration_value_fail_lower(self, path, val, mock_subscribe, mock_connect) -> None:
    ...
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_set_configuration_value_fail_higher(self,  path, val, mock_subscribe, mock_connect) -> None:
        self.dc.set_configuration_value(val, path)
        if path == MIN_MEMORY_MB:
    ...
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.connect')
    @patch('inbm_lib.mqttclient.mqtt.mqtt.Client.subscribe')
    def test_set_configuration_value_success(self, path, val, mock_subscribe, mock_connect) -> None:
        self.dc.set_configuration_value(val, path)
        if path == MIN_MEMORY_MB:
    ...
    def test_execute_success(self, mock_free_space, mock_sub, mock_connect, mock_publish, mock_run) -> None:

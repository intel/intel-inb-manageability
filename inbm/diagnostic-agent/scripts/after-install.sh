...
after_install() {
	...
	chmod 644 $SYSTEMD_DIR/inbm-diagnostic.service

    if [ "$(cat /proc/1/comm)" == "systemd" ]; then
    	echo "Found systemd"
		echo "Activating inbm-diagnostic's apparmor policies"
		apparmor_reload /etc/apparmor.d/usr.bin.inbm-diagnostic
    	# Reload daemon to pick up new changes
		systemctl daemon-reload
		echo "Ran systemctl daemon-reload"
	else
    	echo "No systemd found; run inbm-diagnostic manually"
    fi
}

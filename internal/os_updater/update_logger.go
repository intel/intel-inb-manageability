...
func writeUpdateStatus(status, metadata, errorDetails string) error {
	...
	if _, err := os.Stat(updateStatusLog); os.IsNotExist(err) {
		...
		if err != nil {
			fmt.Printf("Error creating update status log file: %v\n", err)
			return err
		}
		defer file.Close()
	}
	...
	if err != nil {
		fmt.Printf("Error opening update status log file: %v\n", err)
		return err
	}
	defer file.Close()
	...
	if err != nil {
		fmt.Printf("Error writing to update status log file: %v\n", err)
		return err
	}
	return nil
}
